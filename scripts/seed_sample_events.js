...
async function run() {
  ...
  try {
    ...
    // Create 5 sample users (UUIDs)
    const users = Array.from({ length: 5 }).map(() => randomUUID());

    const statements = [];

    // User 0: full funnel (signup -> add1 -> add2 -> aha)
    ...
    // User 3: signup -> first -> aha -> cancellation flow
    ...
    // User 4: multiple events across days
