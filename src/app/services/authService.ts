 ...
 * Creates account with password, sends verification email in background
 * User can access app immediately (soft verification)
 */
export const signUpWithPassword = async (
  ...
  try {
    ...
    // Create user profile (trigger will also create it, but we want to ensure it exists)
    await supabase
      .from('users')
      .upsert(
        {
    ...
    console.error('AuthService.signUpWithPassword catch error:', error);
    return {
      success: false,
      isNewUser: false,
      flow: 'signup',
...
/**
 * Check if email exists and if it has password auth
 */
export const checkEmailExists = async (email: string): Promise<{ exists: boolean; hasPassword: boolean }> => {
  try {
    ...
    // Check if user has password auth (has password auth if provider is 'password' or not specified)
    const hasPassword = data.auth_provider === 'password' || !data.auth_provider;

    return { exists: true, hasPassword };
  } catch (error) {
    console.error('AuthService.checkEmailExists error:', error);
    // On error, try a backup auth-level checks if possible or return false to avoid blocking UI
    return { exists: false, hasPassword: false };
  }
};
...
export const AuthService = {
  // Auth methods - Email + Password Sign Up (NEW)
  signUpWithPassword,
  checkEmailExists,
  ...
  // Auth methods - Email OTP Sign Up (Legacy)
  signUpWithEmail,
  verifySignUpOtp,
