...
import { showLocalNotification, registerServiceWorker } from '../pushService';
...
// ─── Campaign Notification Templates (Tesla-style) ──────────────────────────
// Personalized, action-oriented push notifications
...
export const CAMPAIGN_NOTIFICATION_TEMPLATES = {
  /**
   * Ghost detected: "Found: [service] you forgot. Cancel?"
   * Triggered when a forgotten subscription is detected
   */
  ghost_detected: (data: CampaignNotificationData) => ({
    title: 'Ghost Detected',
    body: `Found: ${data.serviceName || 'service'} you forgot. Cancel?`,
    actionLabel: 'Review Now',
  }),

  /**
   * Guardian alert: "Guardian Alert: [budget] at [X]%"
   * Triggered when budget reaches threshold
   */
  guardian_alert: (data: CampaignNotificationData) => ({
    title: 'Guardian Alert',
    body: `${data.budgetName || 'Budget'} at ${data.budgetPercentage || 80}%`,
    actionLabel: 'Set Guardian',
  }),

  /**
   * Goal progress: "[Goal]: [X]% -> [Y]%. You're flying!"
   * Triggered on significant goal progress
   */
  goal_progress: (data: CampaignNotificationData) => ({
    title: data.goalName || 'Goal Progress',
    body: `${data.fromPercentage || 0}% → ${data.toPercentage || 100}%. You're flying!`,
    actionLabel: 'Ignite Goal',
  }),
};

export type CampaignNotificationType = keyof typeof CAMPAIGN_NOTIFICATION_TEMPLATES;

/**
 * Send a campaign notification through push channel
 * Returns the formatted notification payload
 */
export function getCampaignNotification(
  type: CampaignNotificationType,
  data: CampaignNotificationData
) {
  const template = CAMPAIGN_NOTIFICATION_TEMPLATES[type];
  if (!template) {
    console.warn(`Unknown campaign notification type: ${type}`);
    return null;
  }
 ...
 * Integrates with PushNotificationSender
 */
export async function sendCampaignPushNotification(
  type: CampaignNotificationType,
  data: CampaignNotificationData
): Promise<boolean> {
  const notification = getCampaignNotification(type, data);
  if (!notification) return false;

  try {
    const sender = senders.push;
    await sender.send({
      channel: 'push',
...
class PushNotificationSender implements NotificationSender {
  async send(notification: NotificationPayload): Promise<boolean> {
    try {
      ...
      // Deliver through the service worker (real OS-level notification
      // once the user has granted permission / subscribed via pushService)
      return showLocalNotification(
        notification.title || 'PicksWise',
        notification.body,
        notification.data
      );
    } catch {
    ...
    const alertList = alerts.map(alert => {
      ...
      return `
        <li style="margin-bottom: 12px;">
          <strong>${severityLabel[alert.severity] || '[INFO]'} ${alert.title}</strong>
